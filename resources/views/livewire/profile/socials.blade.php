    ...
    <div class="w-full flex flex-col gap-2">
        ...
        <div class="w-full flex items-center gap-1">
            @foreach(array_column(Socials::cases(), 'value') as $social)
                <button {!! $user->socials()->where('provider', $social)->count() ? 'disabled' : '' !!} wire:loading.attr="disabled" type="button" wire:click="addProvider('{{ $social }}')" class="text-white bg-[#{{ Socials::color($social) }}] hover:bg-[#{{ Socials::color($social) }}]/90 focus:ring-4 focus:outline-none focus:ring-[#{{ Socials::color($social) }}]/50 font-medium rounded-lg text-xs px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#{{ Socials::color($social) }}]/55 mr-2 mb-2 disabled:bg-slate-300 disabled:hover:cursor-default">
