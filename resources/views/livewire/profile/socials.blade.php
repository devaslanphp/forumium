            ...
            @foreach($user->socials as $social)
                <div class="w-full lg:max-w-xs max-w-full bg-white border border-gray-200 rounded dark:bg-gray-800 dark:border-gray-700">
                    <div class="flex justify-end px-4 pt-4">
                        <button id="{{ $social->provider }}-button" data-dropdown-toggle="{{ $social->provider }}" class="inline-block text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-1.5" type="button">
                            <span class="sr-only">Open dropdown</span>
                            <svg class="w-6 h-6" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z"></path></svg>
                        </button>
                        <!-- Dropdown menu -->
                        <div id="{{ $social->provider }}" class="z-10 hidden text-base list-none bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-700">
                            <ul class="py-1" aria-labelledby="{{ $social->provider }}-button">
                                <li>
                                    <button type="button" wire:click="deleteProvider('{{ $social->provider }}')" class="w-full block px-4 py-2 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">
                                        Unlink account
                                    </button>
                                </li>
                            </ul>
                    ...
                    <div class="flex flex-col items-center pb-10">
                        ...
                        <h5 class="mb-1 text-xl font-medium text-gray-900 dark:text-white first-letter:uppercase">{{ $social->provider }}</h5>
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ $social->name }}</span>
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ $social->email }}</span>
                    </div>
                </div>
            @endforeach
        @else
